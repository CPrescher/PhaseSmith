"""Common-model workflow for the IUCr anhydrous trirubidium-citrate/Si holdout."""
...
from ..io.iucr_trirubidium_citrate_silicon import (
...
class IucrTrirubidiumCitrateSiliconResult:
    ...
    def __post_init__(self) -> None:
        ...
        expected = set(IUCR_TRIRUBIDIUM_CITRATE_SILICON_PHASES)
        if (
            ...
            raise ValueError("IUCr trirubidium-citrate/Si result is invalid or non-finite")
        if (
            self.sample_count <= 0
            ...
            raise ValueError("IUCr trirubidium-citrate/Si result counts must be positive")
...
def _manifest(root: Path) -> dict[str, Any]:
    ...
    if record.get("schema_version") != 1 or record.get("scope") != _SCOPE:
        raise ValueError("unsupported IUCr trirubidium-citrate/Si experiment manifest")
    return record


def run_iucr_trirubidium_citrate_silicon_workflow(
    bundle_directory: str | Path,
    *,
    execution: ExecutionPolicy | None = None,
) -> IucrTrirubidiumCitrateSiliconResult:
    """Fit scales/background and diagnose the low-fraction Si position anchor.

    Both phases use the deposited common isotropic base profile. The source-only
    mixing and Stephens terms remain fixed out. The 2.15 wt% Si phase diagnoses
    specimen displacement on its selected windows, but the diagnostic value is
    not applied to the full pattern without a separate transferability gate.
    """
    ...
    if data.shape != (expected_count, 4) or not np.all(np.isfinite(data)):
        raise ValueError("IUCr trirubidium-citrate/Si pattern has unexpected values")
    x, observed, legacy_calculated, legacy_background = data.T
    if np.any(np.diff(x) <= 0.0) or np.any(observed < 0.0):
        raise ValueError("IUCr trirubidium-citrate/Si axis or counts are invalid")
    ...
    profile = instrument_record["common_base_profile"]
    ...
    phases = []
    for phase_id in IUCR_TRIRUBIDIUM_CITRATE_SILICON_PHASES:
