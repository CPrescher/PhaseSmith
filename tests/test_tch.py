...
def test_tch_transform_matches_independent_reference() -> None:
    ...
    for gaussian, lorentzian in zip(
        ...
        actual = rietveld.tch_shape_from_fwhm(gaussian, lorentzian)
...
def test_tch_pure_component_limits() -> None:
    gaussian = rietveld.tch_shape_from_fwhm(0.2, 0.0)
    assert gaussian.total_fwhm == pytest.approx(0.2, rel=1e-15)
    assert gaussian.eta == 0.0

    lorentzian = rietveld.tch_shape_from_fwhm(0.0, 0.3)
...
def test_tch_transform_is_stable_for_extreme_width_scales() -> None:
    ordinary = rietveld.tch_shape_from_fwhm(0.7, 0.3)
    tiny = rietveld.tch_shape_from_fwhm(0.7e-250, 0.3e-250)
    ...
    with pytest.raises(ValueError, match="outside finite range"):
        rietveld.tch_shape_from_fwhm(np.finfo(np.float64).max, np.finfo(np.float64).max)


def test_explicit_gaussian_sigma_helpers_apply_chain_rule() -> None:
    sigma = 0.031
    lorentzian = 0.023
    sigma_shape = rietveld.tch_shape_from_gaussian_sigma(sigma, lorentzian)
    fwhm_shape = rietveld.tch_shape_from_fwhm(
    ...
    sigma_profile = rietveld.profile_tch_from_gaussian_sigma(delta, sigma, lorentzian)
    fwhm_profile = rietveld.profile_tch(
...
def test_tch_profile_matches_reference_and_shape_contracts() -> None:
    gaussian = 0.071
    lorentzian = 0.023
    shape = rietveld.tch_shape_from_fwhm(gaussian, lorentzian)
    delta = np.linspace(-0.8, 0.8, 4_001)
    actual = rietveld.profile_tch(delta, gaussian, lorentzian)
    ...
    half_maximum = rietveld.profile_tch(
...
def test_tch_profile_derivatives_match_centered_differences(parameter: str) -> None:
    ...
    gaussian = 0.083
    lorentzian = 0.037
    baseline = rietveld.profile_tch(delta, gaussian, lorentzian)
    step = 1e-7
    if parameter == "delta":
        plus = rietveld.profile_tch(delta + step, gaussian, lorentzian).value
        minus = rietveld.profile_tch(delta - step, gaussian, lorentzian).value
        analytic = baseline.d_delta
    elif parameter == "gaussian_fwhm":
        plus = rietveld.profile_tch(delta, gaussian + step, lorentzian).value
        minus = rietveld.profile_tch(delta, gaussian - step, lorentzian).value
        analytic = baseline.d_gaussian_fwhm
    else:
        plus = rietveld.profile_tch(delta, gaussian, lorentzian + step).value
        minus = rietveld.profile_tch(delta, gaussian, lorentzian - step).value
...
def test_tch_support_integral_and_centroid() -> None:
    ...
    result = rietveld.accumulate_tch(
        x,
        [position],
        [intensity],
        [gaussian],
...
def test_tch_accumulation_matches_independent_reference() -> None:
    ...
    assert actual.derivatives.local_parameter_names == rietveld.TCH_PARAMETER_ORDER


@pytest.mark.parametrize("parameter", range(4), ids=rietveld.TCH_PARAMETER_ORDER)
def test_tch_accumulation_derivatives_match_finite_differences(parameter: int) -> None:
    ...
    support = 6.123
    baseline = rietveld.accumulate_tch(
        x,
        parameters[:, 0],
        parameters[:, 1],
        parameters[:, 2],
    ...
    def evaluate(values: np.ndarray) -> np.ndarray:
        return rietveld.accumulate_tch(
            x,
            values[:, 0],
            values[:, 1],
            values[:, 2],
    ...
    shape = rietveld.tch_shape_from_fwhm(parameters[0, 2], parameters[0, 3])
...
def test_invalid_tch_widths_fail_clearly(
    ...
    with pytest.raises(ValueError, match=message):
        rietveld.tch_shape_from_fwhm(gaussian, lorentzian)
    with pytest.raises(ValueError, match=message):
        rietveld.profile_tch([0.0], gaussian, lorentzian)
...
def test_invalid_gaussian_sigma_fails_with_sigma_convention(
    ...
    with pytest.raises(ValueError, match=message):
        rietveld.tch_shape_from_gaussian_sigma(sigma, 0.1)
    with pytest.raises(ValueError, match=message):
        rietveld.profile_tch_from_gaussian_sigma([0.0], sigma, 0.1)
