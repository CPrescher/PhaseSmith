...
@pytest.mark.external_oracle
def test_nickel_multibank_tof_geometry_is_on_par_with_pinned_gsasii(
    tmp_path: Path,
) -> None:
    dataset_id = "lanl-nickel-tof"
    verify_validation_dataset(dataset_id, DATA_ROOT / dataset_id)
    output = tmp_path / "nickel-multibank-tof.json"
    command = [
        sys.executable,
    ...
    if process.returncode != 0:
        pytest.fail(
            "nickel multi-bank TOF oracle comparison failed\n"
            f"stdout:\n{process.stdout[-5000:]}\n"
            f"stderr:\n{process.stderr[-5000:]}",
            pytrace=False,
        )
