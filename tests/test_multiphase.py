...
def reflection_batch(
    ...
    return rietveld.ReflectionBatch(
...
def phases() -> tuple[rietveld.Phase, rietveld.Phase]:
    alpha = rietveld.Phase(
        "alpha",
        "Alpha phase",
        reflection_batch("a", np.array([24.0, 42.0, 63.0]), np.array([10.0, 7.0, 4.0])),
        scale=1.25,
        physics=rietveld.IsotropicSizeBroadening(70.0),
    )
    beta = rietveld.Phase(
        "beta",
        "Beta phase",
        ...
        scale=0.65,
        physics=rietveld.IsotropicMicrostrainBroadening(3.0e-4),
    )
    return alpha, beta
...
def test_single_phase_is_exactly_equivalent_to_low_level_cw() -> None:
    ...
    phase = rietveld.Phase("phase-1", "Phase one", reflections)
    actual = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [phase])
    expected = rietveld.accumulate_cw(
        x,
...
def test_fused_multiphase_equals_separate_phase_profiles_and_adds_background_once() -> None:
    ...
    pattern = rietveld.PowderPattern(x, background=background)
    alpha, beta = phases()
    options = rietveld.CalculationOptions(return_phase_components=True)
    fused = rietveld.calculate_pattern(pattern, instrument(), [alpha, beta], options=options)
    alpha_only = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [alpha])
    beta_only = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [beta])
...
def test_phase_and_reflection_labels_preserve_input_order() -> None:
    ...
    alpha, beta = phases()
    result = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [alpha, beta])
    ...
    reversed_result = rietveld.calculate_pattern(
        rietveld.PowderPattern(x), instrument(), [beta, alpha]
    )
...
def test_phase_scale_derivatives_match_centered_differences(phase_index: int) -> None:
    x = np.linspace(20.0, 82.0, 6_201)
    original = phases()
    baseline = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), original)
    step = 1.0e-6
    plus = list(original)
    minus = list(original)
    ...
    finite_difference = (
        rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), plus).y
        - rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), minus).y
    ) / (2.0 * step)
...
def test_zero_scale_phase_has_zero_profile_but_nonzero_scale_derivative() -> None:
    ...
    result = rietveld.calculate_pattern(
...
def test_negative_reflection_intensities_cancel_at_duplicate_positions() -> None:
    ...
    positive = rietveld.Phase(
        "positive",
        "Positive",
        reflection_batch("p", np.array([42.0]), np.array([5.0])),
    )
    negative = rietveld.Phase(
        "negative",
        "Negative",
        reflection_batch("n", np.array([42.0]), np.array([-5.0])),
    )
    result = rietveld.calculate_pattern(
        rietveld.PowderPattern(x), instrument(), [positive, negative]
    )
    np.testing.assert_array_equal(result.profile_y, 0.0)


class CountingProvider:
    descriptor = rietveld.ProviderDescriptor("test.counting", "1")
    ...
    def evaluate(self, context: rietveld.PhysicsContext) -> rietveld.PhysicsContribution:
        self.calls += 1
        return rietveld.IsotropicSizeBroadening(55.0).evaluate(context)
...
def test_prepared_pattern_evaluates_provider_once_and_reuses_flat_inputs() -> None:
    ...
    prepared = rietveld.PreparedPattern(rietveld.PowderPattern(x), instrument(), [phase])
...
def test_stateless_multiphase_dispatches_one_native_accumulation(
    ...
    import rietveld.calculation as calculation

    calls = 0
    native_boundary = calculation.accumulate_cw_contributions

    def counted(*args: object, **kwargs: object) -> rietveld.AccumulationResult:
        nonlocal calls
        calls += 1
    ...
    x = np.linspace(20.0, 82.0, 3_101)
    calculation.calculate_pattern(rietveld.PowderPattern(x), instrument(), phases())
    assert calls == 1
...
def test_phase_specific_provider_rows_are_prefixed_and_finite_differenced() -> None:
    ...
    alpha, beta = phases()
    size = 70.0
    baseline = rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [alpha, beta])
    step = 2.0e-4
    plus = replace(alpha, physics=rietveld.IsotropicSizeBroadening(size + step))
    minus = replace(alpha, physics=rietveld.IsotropicSizeBroadening(size - step))
    finite_difference = (
        rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [plus, beta]).y
        - rietveld.calculate_pattern(rietveld.PowderPattern(x), instrument(), [minus, beta]).y
    ) / (2.0 * step)
...
def test_pattern_and_identity_validation_fail_at_the_boundary() -> None:
    with pytest.raises(ValueError, match="strictly increasing"):
        rietveld.PowderPattern([1.0, 1.0])
    with pytest.raises(ValueError, match="uncertainty"):
        rietveld.PowderPattern([1.0, 2.0], uncertainty=[1.0, 0.0])
    with pytest.raises(ValueError, match="boolean"):
        rietveld.PowderPattern([1.0, 2.0], mask=[0, 1])
    with pytest.raises(ValueError, match="reflection_ids"):
        rietveld.ReflectionBatch(["same", "same"], [[1, 0, 0], [0, 1, 0]], [1, 1], [20, 30], [1, 1])
    alpha = phases()[0]
    with pytest.raises(ValueError, match="phase_id"):
        rietveld.calculate_pattern(
            rietveld.PowderPattern([20.0, 21.0]), instrument(), [alpha, alpha]
        )
    with pytest.raises(ValueError, match="phase scale"):
        replace(alpha, scale=-1.0)
    with pytest.raises(KeyError, match="no diagnostic"):
        rietveld.calculate_pattern(
            rietveld.PowderPattern([20.0, 21.0]), instrument(), [alpha]
        ).phase_y("alpha")
