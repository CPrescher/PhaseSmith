...
native runners rather than repeating the paired GSAS-II process. After matching
the scripting optimizer's phase-scale conditioning, weighted free-coordinate
linearization, accepted-trial reuse, and physical Bruckner-width conversion,
QARR takes 0.645 s, PbSO4 neutron 2.337 s, and PbSO4 fixed-doublet X-ray
4.223 s on the development host. The corresponding Poisson Rwp values are
19.844%, 4.217%, and 10.346%. Before native general objectives reused their
bounded analytical linearizations, the same recorded QARR and X-ray runs took
13.02 s and 157.44 s; neutron took about 17.56 s. The current changes are
therefore about 20.2x, 7.5x, and 37.3x faster respectively.

A fresh Python scripting X-ray reference takes 3.560 s at 10.34604% Rwp; the
pure-Rust runner takes 4.223 s at 10.34601% Rwp. The native result is therefore
scientifically coincident and about 19% slower on this host. Its final stage
accepts 61 iterations versus Python's 48 because small floating-point
accumulation differences continue to expose strictly decreasing steps near the
flat minimum, not because it uses a different optimizer or X-ray model. The
historical 12.943 s GSAS-II number is a joint two-histogram workflow and must
not be presented as a like-for-like ratio against any one of these native
runners.
