...
fn native_manifest_schema_is_valid_json_and_tracks_the_wire_version() {
    let schema_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../schemas/native-project-v7.schema.json");
    let schema: serde_json::Value =
        serde_json::from_slice(&fs::read(schema_path).unwrap()).unwrap();
    assert_eq!(schema["properties"]["format_version"]["const"], 7);
    assert_eq!(
...
fn old_projects_migrate_but_each_version_requires_its_analysis_fields() {
    ...
    let mut manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(&manifest_path).unwrap()).unwrap();
    manifest.as_object_mut().unwrap().remove("pawley_analyses");
    manifest
        .as_object_mut()
        .unwrap()
        .remove("tof_pawley_analyses");
    ...
    manifest["structural_tof_multibank_analyses"] = serde_json::json!([]);
    fs::write(&manifest_path, serde_json::to_vec(&manifest).unwrap()).unwrap();
    assert_eq!(
        load_project(&directory, ProjectReadLimits::default()).unwrap(),
        expected
