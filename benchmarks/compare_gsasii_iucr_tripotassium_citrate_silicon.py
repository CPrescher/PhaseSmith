#!/usr/bin/env python3
"""Compare PhaseSmith and pinned GSAS-II on the tripotassium-citrate/Si holdout."""
...
def comparison_checks(
    ...
    measured = {
        ...
        "phase_fraction_delta": max(
            ...
            for name in IUCR_TRIPOTASSIUM_CITRATE_SILICON_PHASES
        ),
...
def main() -> None:
    ...
    source = arguments.data_directory / "wm5301sup1.cif"
    with tempfile.TemporaryDirectory(prefix="phasesmith-iucr-k-si-") as name:
        temporary = Path(name)
        bundle = temporary / "bundle"
        manifest_path = convert_iucr_tripotassium_citrate_silicon_bundle(source, bundle)
        phasesmith_result = run_iucr_tripotassium_citrate_silicon_workflow(bundle).to_record()
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "external GSAS-II tripotassium-citrate/Si worker failed\n"
                f"stdout:\n{completed.stdout}\nstderr:\n{completed.stderr}"
            )
        ...
        report = {
            "schema_version": 1,
            "scope": "iucr_tripotassium_citrate_silicon_transferability_comparison",
            ...
            "common_model": {
                ...
                "fixed_parameters": [
                    "dominant-phase U/V/W/X/Y and matched SH/L=0.0368",
                    "instrument zero, specimen displacement=0, and Si lattice",
                    "all phase size/strain/orientation parameters",
                ],
            },
            ...
            "interpretation": [
                ...
                "The 1.44 wt% silicon windows do not transfer the displacement-calibration "
                "contract accepted for the sodium holdout; their estimates are diagnostic only.",
                "Both common-subset fits remain far from the richer deposited GSAS model, so "
                "this is not physical-model acceptance or justification for new profile terms.",
            ],
        }
    print(
    ...
    if validation["status"] != "qualified_pass":
        raise RuntimeError("tripotassium-citrate/Si validation did not match expected outcome")
